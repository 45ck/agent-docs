 ...
 *
 * @spec SPEC-001
 * @implements src/v2/policy.ts#evaluateSpec
 */
import type {
...
export function evaluateSpec(
  ...
  // Merge kind-specific overrides
  const effectiveRule = mergeKindOverrides(stateRule, spec.kind, policy);

  // Get required evidence from spec-level overrides or policy defaults
  ...
  // Check for warn-missing obligations (things not required but worth noting)
  if (effectiveRule.warnMissing) {
    for (const obligation of effectiveRule.warnMissing) {
      if (required[obligation] !== undefined) continue; // Already handled as required
...
function resolveRequirements(
  ...
  // Spec-level required_evidence overrides policy defaults
...
function evaluateObligation(
  ...
  if (relevantClaims.length === 0) {
    // No claims at all — check for waiver
  ...
  // Insufficient evidence — check for waiver
  const waiver = findWaiver(waivers, obligation);
  if (waiver && rule.allowWaiverFor?.includes(obligation)) {
    return {
      specId,
