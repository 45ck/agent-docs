...
export class FileProvider implements Provider {
  ...
  /**
   * The file provider doesn't discover new subjects — it validates existing
   * claims by checking whether referenced files exist on disk.
   */
  ...
  /**
   * Validate a claim by checking if the file referenced in the subject exists.
   * If the file exists, returns E1 evidence. If not, returns nothing.
   */
...
/**
 * Extract the file path from a subject ID.
 *
 * Subject IDs look like:
 *   symbol:ts:src/auth/login.ts#login  -> src/auth/login.ts
 *   file:generic:infra/backup.sh       -> infra/backup.sh
 *   test:vitest:tests/auth.spec.ts     -> tests/auth.spec.ts
 *   symbol:annotation:src/foo.ts#bar   -> src/foo.ts
 */
function extractFilePath(subjectIdOrRaw: string): string | null {
  // Try parsing as a subject ID
  try {
    const parsed = parseSubjectId(subjectIdOrRaw);
    const identity = parsed.identity;
    // Strip fragment (symbol after #)
    const hashIdx = identity.indexOf('#');
    return hashIdx >= 0 ? identity.slice(0, hashIdx) : identity;
  } catch {
    // Not a valid subject ID — try treating as a raw path
    const hashIdx = subjectIdOrRaw.indexOf('#');
    const candidate = hashIdx >= 0 ? subjectIdOrRaw.slice(0, hashIdx) : subjectIdOrRaw;
    // Basic sanity: should look like a file path
